...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Processar eventos de pagamento de fatura bem-sucedido (RENOVAÇÕES)
      ...
      if (invoice.subscription && invoice.customer) {
        ...
        if (userError || !userData) {
          console.error(
            "Usuário não encontrado para o customer ID:",
            customerId,
          );
          return NextResponse.json(
        ...
        if (updateResult.error) {
          console.error(
            "Erro ao atualizar dados de renovação:",
            updateResult.error,
          );
          return NextResponse.json(
        ...
        console.log(
          `Assinatura renovada com sucesso para o usuário: ${userData.id}, nova data de término: ${subscriptionEndDate.toISOString()}`,
        );
      }
    }
    ...
    // Processar alterações de status da assinatura
      ...
      console.log(
        `Usuário encontrado: ${userData.id}, status atual: ${userData.subscription_status}`,
      );
      ...
      if (isCanceled) {
        console.log(
          `Detectado cancelamento de assinatura no evento ${event.id}`,
        );
        ...
        if (updateResult.error) {
          console.error(
            "Erro ao atualizar status após cancelamento:",
            updateResult.error,
          );
          return NextResponse.json(
            { error: "Database update failed" },
        ...
        console.log(
          `Assinatura cancelada para o usuário: ${userData.id} (via evento updated)`,
        );
      }
      // Caso contrário, é uma atualização normal (não cancelamento)
      else {
        console.log(
          `Atualizando assinatura para status: ${subscription.status}`,
        );
        ...
        console.log(
          `Status da assinatura atualizado para: ${subscription.status}`,
        );
      }
    }
    ...
    // Lidar com falhas de pagamento
      ...
      if (invoice.subscription && invoice.customer) {
        ...
        if (userData) {
          ...
          console.log(
            `Falha no pagamento registrada para o usuário: ${userData.id}`,
          );
        }
      }
    }
    ...
    // Lidar com o cancelamento da assinatura
      ...
      if (userError || !userData) {
        console.error(
          "Usuário não encontrado para o customer ID (cancelamento):",
          customerId,
        );
        return NextResponse.json({ error: "User not found" }, { status: 404 });
      }
